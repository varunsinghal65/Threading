 ...
 * We used semaphore as condition variable.
 */
public class MatricesMultiplicationPipelinePCBackPreassureSolution2 {
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (mPair.m1 == null || mPair.m2 == null) {
                    ...
                    System.out.println("Producerthread took: " + Duration.between(start, end).toMillis());
                    break;
                }
                try {
                    queue.add(mPair);
    ...
    private static class ThreadSafeMatricesQueue {
        ...
        private Semaphore empty = new Semaphore(QUEUE_CAPACITY_TO_AVOID_PREASSURE_ON_CONSUMER);
        private Lock sharedResourceLock = new ReentrantLock();

        public void add(MatrixPair matrixPair) throws InterruptedException {
        ...
        public MatrixPair remove() throws InterruptedException {
        ...
        public void terminate() {
            synchronized (this) {
                isTerminated = true;
                System.out.println("ThreadSafeQueue : Number of threads waiting to acquire FULL sempahore: " + full.getQueueLength());
                full.release(full.getQueueLength());
            }
        }
    }
